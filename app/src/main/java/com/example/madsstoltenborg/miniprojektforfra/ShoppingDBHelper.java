...
//
            ...
            db.execSQL("CREATE VIEW GROCERYLIST_PRODUCTS_VIEW AS " +
                 "SELECT " +
                 "A._id AS _id, A.GROCERYLISTS_ID AS GROCERYLIST_ID, A.QUANTITY AS QUANTITY, A.BOUGHT, B.PRICE AS PRICE, C.NAME AS PRODUCTNAME, C.VOLUME AS VOLUME, D.NAME AS SHOPNAME " +
                "FROM GROCERYLISTS_PRODUCTS A, SHOP_PRODUCTS B, PRODUCTS C, SHOPS D " +
                "WHERE A.SHOP_PRODUCT_ID = B._id AND B.PRODUCT_ID = C._id AND B.SHOP_ID = D._id;");
        }
